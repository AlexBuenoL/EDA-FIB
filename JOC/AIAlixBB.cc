...
struct PLAYER_NAME : public Player {
  ...
  virtual void play () {
      ...
      for (int id : UnitatsVives) {
          ...
          for (auto d : dirs) {
              Pos p = pUV + d;
              if (pos_ok(p) and cell(p).type != Waste and cell(p).id != -1 and unit(cell(p).id).type == Dead and unit(cell(p).id).rounds_for_zombie <= 3) {
                  move(id,d);
              }
          }
          ...
          if (distDead <= distEnem and distDead <= distZombie and distDead <= distFood and distDead <= unit(cell(PI.pos_dead).id).rounds_for_zombie)
              ...
              if (distFood <= distEnem and distFood <= distZombie) move(id, dirFood);
          
              // Si el menjar no és el que està més a prop
              else {
              
                  if (distZombie == 2) {
                      if (distFood - distEnem <= 5) move(id, dirFood);
                      else move(id, dirEnem);
                  } else {
                      if (distFood - distEnem <= 5 and distFood - distZombie <= 5) move(id, dirFood);
                      else {
                          if (distZombie < distEnem) move(id, dirZombie);
                          else move(id, dirEnem);
                      }
                  }
              }
